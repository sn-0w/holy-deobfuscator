...
namespace holydeobfuscator
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                if (type.Namespace.Length > 10) type.Namespace = "Default_Namespace";
                type.Name = "Type_" + typecount;
                typecount++;
                int methodcount = 0;

                foreach (var method in type.Methods)
                {
                    method.Name = "Method_" + methodcount;
                    methodcount++;
                }
            }
           ...
           sizeoffix(md);
            ...
            foreach (TypeDef type in md.GetTypes())
            {
                ...
                foreach (MethodDef method in type.Methods)
                {
                    ...
                    for (int i = 0; i < method.Body.Instructions.Count; i++)
                    {
                        try
                        {
                            var methodus = md.Import(typeof(System.Console).GetMethod("WriteLine", new Type[] { typeof(int) }));
                            var instr = method.Body.Instructions;
                            if (instr[i].OpCode == OpCodes.Call)
                            {
                                if (instr[i].Operand.ToString().Contains("Environment::Exit"))
                                {
                                    instr[i].Operand = methodus;
                                }
                            }
                            if (instr[i].OpCode == OpCodes.Ldstr && instr[i + 1].OpCode == OpCodes.Call)
                            {
                                IMethod invoker = (IMethod)instr[i + 1].Operand;
                                MethodDef invoker2 = invoker.ResolveMethodDefThrow();
                                if (invoker2.ReturnType == md.CorLibTypes.String && invoker2.HasParams() && invoker2.GetParams().Count() == 1 && invoker2.GetParams().First().ToString().Contains("String"))
                                {
                                   var invokey = assembly.ManifestModule.ResolveMethod(invoker.MDToken.ToInt32());
                                    instr[i].Operand = invokey.Invoke(null, new object[] { instr[i].Operand });
            ...
            md.Write(args[0].Replace(".exe", "-obf.exe"));
            Console.ReadLine();
        }

        public static void sizeoffix(ModuleDefMD module)
        {
            try
            {
                int decrypted = 0;
